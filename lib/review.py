...
class Review:
    ...
    def employee_id(self, employee_id):
        sql = """
            ...
            WHERE id = ? 
            """
        given_employee = CURSOR.execute(sql, (employee_id,)).fetchall()
        print("Given Employee: ", given_employee)
        if given_employee:
            self._employee_id = employee_id
        else:
            raise ValueError(
                "Employee whose ID is given must be in database."
            )
    ...
    def instance_from_db(cls, row):
        ...
        if review:
            ...
            review.create(row[1], row[2], row[3])
